function WhatsAppSOASM() {
    var context = nlapiGetContext();
    var isRescheduled = false;
    // Retrieve the parameter value using the script context
    var whatsInternalID = context.getSetting('SCRIPT', 'custscript_whatsapp_soa_sm_params');
                ...
                for (var j = 0; j < subCustomerSearch.length; j++) {
                    ...
                    if (nlapiGetContext().getRemainingUsage() < 200) {
                        ...
                        var status = nlapiScheduleScript(
                            'customscript_whatsapp_soa_sm',
                            'customdeploy_whatsapp_soa_sm',
                            {
                                custscript_whatsapp_soa_sm_params: whatsInternalID,
                            }
                        );
